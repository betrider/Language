...
BEGIN
	...
	BEGIN --3.notePad
		...
		!!!테이블 정보 조회(생성된 날짜,수정된 날짜 등등)
			SELECT * FROM USER_OBJECTS WHERE OBJECT_TYPE = 'TABLE'
			
		!!!테이블 명 조회
			SELECT TABLE_NAME FROM ALL_TABLES WHERE OWNER = '유저명'
		...
		*ROLLUP 쿼리
			...
			*테이블 명세서 쿼리
			SELECT COLUMN_ID AS SEQ
					...
					,REPLACE (C.COMMENTS, ',', '&') COMMENTS
					,A.DATA_TYPE
					,A.DATA_LENGTH
					, (SELECT DECODE(NVL(D.POSITION, ''),'','','PK')
						 FROM ALL_CONS_COLUMNS D, all_constraints E
						WHERE     D.OWNER = UPPER ('HSLP_FIS')
							  AND D.OWNER = A.OWNER
			   ...
			   WHERE     A.OWNER = UPPER ('HSLP_FIS')
					 AND B.OWNER = UPPER ('HSLP_FIS')
					 AND C.OWNER = UPPER ('HSLP_FIS')
					 AND A.OWNER = B.OWNER
					 AND A.OWNER = C.OWNER
					 AND A.TABLE_NAME = B.TABLE_NAME
					 AND A.TABLE_NAME = C.TABLE_NAME
					 AND A.COLUMN_NAME = C.COLUMN_NAME
					 AND A.TABLE_NAME = UPPER ('FMS_BKG_ITEM')
			ORDER BY 1
